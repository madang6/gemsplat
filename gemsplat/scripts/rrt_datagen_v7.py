...
class RRT:
    ...
    def remove_intersecting_edges(self):
        ...
        # Build a mapping from hashable edge keys to actual edges
        edge_mapping = {}
        for edge in self.edges:
            edge_key = self.edge_key(edge)
            edge_mapping[edge_key] = edge
            ...
            for edge2 in edge_list[i+1:]:
                ...
                if self.do_edges_intersect(edge1_start, edge1_end, edge2_start, edge2_end):
                    ...
                    if edge_to_remove is None:
                        # Do not remove any edge if both are essential
                        continue
                    # Convert edge to hashable key
                    edge_to_remove_key = self.edge_key(edge_to_remove)
    ...
    def enforce_minimum_edge_separation(self, min_edge_separation):
        ...
        for i, edge1 in enumerate(edge_list):
            for edge2 in edge_list[i+1:]:
                edge1_key = self.edge_key(edge1)
                edge2_key = self.edge_key(edge2)
    ...
    def do_edges_intersect(self, a_start, a_end, b_start, b_end):
        """
        Checks if two edges (line segments) intersect in 2D space.
        """
    ...
    def select_edge_to_remove(self, edge1, edge2):
        """
        Decides which edge to remove between two conflicting edges.
        Returns None if neither edge should be removed.
        """
        optimal_path_edges = self.get_optimal_path_edges()
        optimal_edges_keys = set(self.edge_key(e) for e in optimal_path_edges)

        edge1_key = self.edge_key(edge1)
        edge2_key = self.edge_key(edge2)

        if edge1_key in optimal_edges_keys and edge2_key not in optimal_edges_keys:
            return edge2  # Remove edge2
        elif edge2_key in optimal_edges_keys and edge1_key not in optimal_edges_keys:
            return edge1  # Remove edge1
        elif edge1_key not in optimal_edges_keys and edge2_key not in optimal_edges_keys:
            # Neither edge is on the optimal path; remove the one with higher cost
            edge1_cost = self.get_edge_cost(edge1)
            edge2_cost = self.get_edge_cost(edge2)
            return edge1 if edge1_cost > edge2_cost else edge2
        else:
            # Both edges are on the optimal path; do not remove either
            return None
    ...
    def remove_edge(self, edge):
        ...
        self.edges = [e for e in self.edges if not (
            np.array_equal(e[0], start_pos) and np.array_equal(e[1], end_pos)
        )]
        ...
        if start_node and end_node:
            ...
            if start_node is end_node.parent:
                ...
                # Reconnect the end_node to maintain tree connectivity
                self.reconnect_node(end_node)
    
    def reconnect_node(self, node):
        """
        Attempts to reconnect a disconnected node to the tree by finding the nearest node
        that does not violate any constraints.
        """
        node_positions = np.array([n.position for n in self.nodes if n is not node])
        if len(node_positions) == 0:
            return  # No nodes to connect to
        tree = cKDTree(node_positions)
        _, idx = tree.query(node.position)
        nearest_node = self.nodes[idx]
        if self.is_collision_free(nearest_node.position, node.position):
            node.parent = nearest_node
            nearest_node.children.append(node)
            node.cost = nearest_node.cost + np.linalg.norm(node.position - nearest_node.position)
            self.edges.append((nearest_node.position, node.position, nearest_node.color))
        else:
            print(f"Node at position {node.position} could not be reconnected due to collision.")
    ...
    def get_optimal_path_edges(self):
        ...
        for node in path_nodes:
            if node.parent:
                edge = (node.parent.position, node.position)
    ...
    def select_node_to_remove(self, node1, node2):
        """
        Decides which node to remove between two conflicting nodes.
        Prefers to keep nodes that are part of the optimal path.
        """
        optimal_path_nodes = self.get_best_path_nodes()
        if node1 in optimal_path_nodes and node2 not in optimal_path_nodes:
            ...
            return node1  # Remove node1
        else:
            # Compare node depths or costs
            depth1 = self.get_node_depth(node1)
            depth2 = self.get_node_depth(node2)
            if depth1 < depth2:
                return node2  # Keep node1 (shallower depth)
            elif depth2 < depth1:
                return node1  # Keep node2 (shallower depth)
            else:
    ...
    def get_node_depth(self, node):
        """
        Returns the depth of a node in the tree (number of edges from the root).
        """
        depth = 0
        current_node = node
    ...
    def point_to_segment_distance(self, point, seg_start, seg_end):
        """
        Computes the distance from a point to a line segment in 2D space.
        """
    ...
    def edge_key(self, edge):
        """
        Generates a consistent, hashable key for an edge.
        """
        start_pos = tuple(np.round(edge[0], decimals=5))
        end_pos = tuple(np.round(edge[1], decimals=5))
        # Ensure the key is order-independent
    ...
    def find_node_by_position(self, position):
        """
        Finds a node in self.nodes by its position.
        """
        for node in self.nodes:
